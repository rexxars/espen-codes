...
import {LatLng, Map, type LeafletEvent, type LocationEvent} from 'leaflet'
import {Circle, MapContainer, Polyline, TileLayer, useMapEvent} from 'react-leaflet'
import 'leaflet/dist/leaflet.css'

import {MAPBOX_API_KEY} from '../constants'
import type {SfMapProps, UserLocation} from '../types/map'
...
const sfBoundaryCoordinates: Array<[number, number]> = [
...
const tier1Coordinates: Array<[number, number]> = [
...
const sfBoundaryLine = sfBoundaryCoordinates.map(toLatLng)
const tier1Line = tier1Coordinates.map(toLatLng)

const sfCenter = new LatLng(37.76578720301821, -122.44688605344325)

export default function SanFranMap(props: SfMapProps): React.JSX.Element {
  const [userLocation, setUserLocation] = useState<UserLocation | null>(null)
  const hasMoved = useRef(false)
  const [mapRef, setMapRef] = useState<Map | null>(null)

  const handleUserLocation = useCallback((position: GeolocationPosition): void => {
  ...
  const onLocationFound = useCallback(
    (event: LocationEvent): void => {
      if (!hasMoved.current && mapRef) {
        mapRef.setView(event.latlng)
      }
    },
    [mapRef],
  )
  ...
  }, [handleUserLocation, mapRef, onLocationFound])

  const handleMove = useCallback((_event: LeafletEvent): void => {
  ...
  return (
    <div style={{position: 'relative'}}>
      ...
      >
        ...
        <Polyline positions={props.paths} />
...
function MapEventListener(props: {onMove: (event: LeafletEvent) => void}): null {
